  ...
  main(int argc, char *argv[])
{
  ...
  try {
	  ...
	  // Complementation of the input image and initialize all the pixels of output image to 0
	  for (n = 0; n < num_comps; n++)
		  for (r = 0; r < height; r++)
			  for (c = 0; c < width; c++) {
				  input_comps[n].buf[r*input_comps[n].stride + c] = (input_comps[n].buf[r*input_comps[n].stride + c] == 255) ? 0 : 255;
				  output_comps[n].buf[r*output_comps[n].stride + c] = 0;
			  }
	  
	  for (n = 0; n < num_comps; n++)
		  input_comps[n].symmetric_extension();

	  // Apply the opening operation with the mirror image version of SE
	  for (n = 0; n < num_comps; n++) {
		  for (r = 0; r < height; r++) {
			  for (c = 0; c < width; c++) {
	  ...
	  for (n = 0; n < num_comps; n++) 
		  for (r = 0; r < height; r++) 
			  for (c = 0; c < width; c++) 
				  output_comps[n].buf[r*output_comps[n].stride + c] = (output_comps[n].buf[r*output_comps[n].stride + c] == 255) ? 0 : 255;
