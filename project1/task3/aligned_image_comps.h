...
struct my_aligned_image_comp {
    ...
    void filter(my_aligned_image_comp *in, int filter_length, int pattern);
       /* Separable implementation of two dimensional filtering, mapping `in' to the
          current image component.  This function is implemented in
          "filtering_main.cpp". */
	void filter_shift(float *mirror_psf, int FILTER_EXTENT, float sigma);
		/* Delay the basic sinc(s) function by sigma */
