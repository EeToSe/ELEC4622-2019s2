...
#define FILTER_TAPS 2*FILTER_EXTENT+1
	...
	for (int t = -filter_length; t <= filter_length; t++) {
		mirror_kernal_1[t] = (t == 0) ? 1 : 0;
		mirror_kernal_2[t] = SINC(t + 0.4f)*0.5f*(1 + cosf(PI*(t + 0.4f) / (filter_length + 0.6f)));
		mirror_kernal_3[t] = SINC(t - 0.2f)*0.5f*(1 + cosf(PI*(t - 0.2f) / (filter_length + 0.8f)));
		mirror_kernal_4[t] = SINC(t + 0.2f)*0.5f*(1 + cosf(PI*(t + 0.2f) / (filter_length + 0.8f)));
		mirror_kernal_5[t] = SINC(t - 0.4f)*0.5f*(1 + cosf(PI*(t - 0.4f) / (filter_length + 0.6f)));
	}

	float gain0 = 0, gain1 = 0, gain2 = 0, gain3 = 0, gain4 = 0;
	for (int t = -FILTER_EXTENT; t <= filter_length; t++) {
		gain0 += mirror_kernal_1[t];
		gain1 += mirror_kernal_2[t];
		gain2 += mirror_kernal_3[t];
		gain3 += mirror_kernal_4[t];
		gain4 += mirror_kernal_5[t];
	}

	for (int t = -FILTER_EXTENT; t <= filter_length; t++) {
		mirror_kernal_1[t] = mirror_kernal_1[t] / gain0;
		mirror_kernal_2[t] = mirror_kernal_2[t] / gain1;
		mirror_kernal_3[t] = mirror_kernal_3[t] / gain2;
		mirror_kernal_4[t] = mirror_kernal_4[t] / gain3;
		mirror_kernal_5[t] = mirror_kernal_5[t] / gain4;
	}
