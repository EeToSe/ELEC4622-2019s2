...
void my_aligned_image_comp::filter(my_aligned_image_comp *in, int H, int mode)
{
	...
	float *one_buf = new float [FILTER_TAPS];
	float *one_psf = one_buf + (FILTER_DIM*FILTER_EXTENT) + FILTER_EXTENT;
		...
		for (cc = 0; cc < in->width; cc++) {
			for (r = 0, rr = 0; r < height; r += 5, rr += 2) {
				for (int i = 0; i < num_kernels; i++) {
					...
					float sum = 0.0F;
					for (int t = -FILTER_EXTENT; t <= FILTER_EXTENT; t++) {
						sum += ip[t*in->stride] * mirror_psf[1][t];
					}
					*op = sum;
				}
			}
	...
	for (r = -FILTER_EXTENT; r <= FILTER_EXTENT; r++)
		for (c = -FILTER_EXTENT; c <= FILTER_EXTENT; c++) {
			one_psf[r*FILTER_DIM + c] = mirror_psf[1][c] * mirror_psf[1][r];
		}
			...
			for (c = 0, cc = 0; c < width; c += 5, cc += 2) {
				for (int i = 0; i < num_kernels; i++) {
					for (int j = 0; j < num_kernels; j++) {
						int nearest_i = int(i*0.4F + 0.5F); // find the nearest interger location
						int nearest_j = int(j*0.4F + 0.5F);
						float *ip = in->buf + ((rr + nearest_i)*in->stride) + cc + nearest_j;
						float *op = this->buf + (r + i) * stride + c + j;
						float sum = 0.0F;
						for (int y = -FILTER_EXTENT; y <= FILTER_EXTENT; y++) {
							for (int x = -FILTER_EXTENT; x <= FILTER_EXTENT; x++) {
								sum += ip[y*in->stride + x] * one_psf[y*FILTER_DIM + x];
							}
						}
						*op = sum;
					}
